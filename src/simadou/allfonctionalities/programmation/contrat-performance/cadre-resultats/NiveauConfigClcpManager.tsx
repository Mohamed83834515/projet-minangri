...
import {
  useCreateNiveauConfigClcp,
  useDeleteNiveauConfigClcp,
  useGetNiveauxConfigClcp,
  useUpdateNiveauConfigClcp,
} from '@/simadou/allHooks/admin/cadreLogiqueClcpHooks'
import {
  encodeNiveauClcpCode,
  parseNiveauClcpCode,
...
function toRow(n: NiveauConfigClcp): NiveauRow {
  const { libelle, codeNumber } = parseNiveauClcpCode(n.code_number_ncl)
  return {
    id: n.id_niveau_ncl,
    libelle,
    codeNumber,
...
export default function NiveauConfigClcpManager({ idContrat }: Props) {
  const { data: me } = useMe()
  const { data: niveaux = [], isLoading } = useGetNiveauxConfigClcp(idContrat)
  const createMutation = useCreateNiveauConfigClcp(idContrat)
  const updateMutation = useUpdateNiveauConfigClcp(idContrat)
  const deleteMutation = useDeleteNiveauConfigClcp(idContrat)

  const sorted = useMemo(
    () => [...niveaux].sort((a, b) => a.nombre_ncl - b.nombre_ncl),
    [niveaux]
  )
  ...
  const onSave = async () => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        order += 1
        const payload = {
          code_number_ncl: encodeNiveauClcpCode(
            row.libelle.trim(),
            Number(row.codeNumber)
          ),
          nombre_ncl: order,
          etat: true,
          contrat: idContrat,
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setRows((p) =>
                        p.map((r, i) =>
                          i === index
                            ? { ...r, codeNumber: Number(e.target.value) }
                            : r
                        )
                      )
                    }
