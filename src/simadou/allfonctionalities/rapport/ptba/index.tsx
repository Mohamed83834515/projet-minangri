...
import { filterPtbasByVersion, resolvePtbaActiviteId } from '@/simadou/allfonctionalities/rapport/rapportTableUtils'
import { EMPTY_PTBA_LIST } from '@/simadou/allfonctionalities/rapport/rapportTableUtils'
...
export default function RapportPtbaPage() {
    ...
    const { data: ptbasRaw = EMPTY_PTBA_LIST, isLoading: ptbasLoading } = useGetPtbas()
    const { data: allTaches = [], isLoading: tachesLoading } = useGetAllTachesActivite()
    const { data: allIndicateurs = [], isLoading: indicateursLoading } = useGetAllIndicateursTache()
    const { data: allCouts = [], isLoading: coutsLoading } = useGetCoutsUnitaires()
    ...
    const taches = useMemo(() => {
        ...
        return allTaches.filter((tache) => {
            const id = typeof tache.id_activite === 'number'
                ? tache.id_activite
    ...
    const indicateurs = useMemo(() => {
        ...
        return allIndicateurs.filter((indicateur) => {
            return indicateur.id_activite != null && activiteIdSet.has(indicateur.id_activite)
        })
    ...
    const couts = useMemo(() => {
        ...
        return allCouts.filter((cout) => {
            const id = typeof cout.ptba_activite === 'number'
                ? cout.ptba_activite
    ...
    const isLoading = ptbasLoading || tachesLoading || indicateursLoading || coutsLoading
