...
import CibleCmrProjetDetailView from '@/simadou/allfonctionalities/projets/detail/cmrIndicators/CibleCmrProjetDetailView'
import CibleCmrProjetFormPanel from './CibleCmrProjetFormPanel'
...
export default function CiblesCmrDialog({ open, onOpenChange }: OpenProps) {
  const { data: cibles = [], dataUpdatedAt } = useGetAllCiblesCmrProjet()
  ...
  return (
    <>
        ...
        >
          <DialogHeader className='border-b px-6 py-4'>
            ...
            <DialogDescription className='px-6 pb-0'>
              Gestion globale des cibles CMR projet.
            </DialogDescription>
          </DialogHeader>

          <div className='px-6 py-4'>
            <GenericTable<CibleCmrProjet>
              key={`cibles-cmr-global-${dataUpdatedAt}-${cibles.length}`}
              data={cibles}
              columns={columns}
              search={tableState.search}
              navigate={tableState.navigate}
              searchKey='code_indicateur_crp'
              searchPlaceholder='Filtrer par indicateur…'
              urlFilterConfig={[
                {
              ...
              emptyMessage='Aucune cible CMR projet'
            />
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={open && modal === 'form'} onOpenChange={(o) => !o && backToList()}>
        <DialogContent className='gap-0 overflow-hidden p-0 sm:max-w-2xl' aria-describedby={undefined}>
          <DialogHeader className='border-b px-6 py-4'>
            <DialogTitle>
              {selectedCible
