...
export const activiteService = {
  ...
  async getAll(id_projet: number): Promise<Activite[]> {
    const response = await apiClient.request<Activite[]>("/activite_programme/");
  ...
  async create(data: Activite): Promise<Activite> {
    const { ...form } = data;
    return await apiClient.request<Activite>("/activite_projet/", {
      method: "POST",
      data: form,
    });
  },
  ...
  async update(data: Activite): Promise<Activite> {
    ...
    return await apiClient.request<Activite>(`/activite_programme/${id_activite_projet}/`, {
      method: "PUT",
      data: form,
    });
  },
